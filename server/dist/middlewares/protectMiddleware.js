 ...
 */
export const createProtectMiddleware = (authCodeCollection, userCollection) => {
    return async (request, reply) => {
        ...
        try {
            ...
            if (!csrfTokenFromHeader)
                throw new Error('Unauthorized. CSRF token missing from header.');
            // Verify tokens
            ...
            const isValidatedHeader = validateCSRFToken(csrfTokenFromHeader?.toString() || '', userAuth._id.toString());
            const isValidatedCookie = validateCSRFToken(csrf_token?.toString() || '', userAuth._id.toString());
...
async function fetchNewRefreshToken(userAuth, request, reply, authCodeCollection) {
    const { refreshToken } = request.cookies || {};
    try {
        // Generate new tokens
        const newRefreshToken = generateToken(userAuth.userId, '7d', 'refreshToken');
        ...
        await authCodeCollection.updateOne({ _id: userAuth._id }, {
            $set: {
...
async function refreshAccessToken(userAuth, request, reply) {
    try {
