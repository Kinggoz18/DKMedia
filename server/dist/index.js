...
const connectToDatabase = async () => {
    try {
        ...
        if (!client)
            throw new Error("Something went wrong while trying to connect to Mongodb");
        // Add mongodb plugin
...
export const startServer = async (server) => {
    try {
        if (BASE_PATH === "" || !BASE_PATH)
            throw new Error("Base path url is undefined");
        ...
        const build = (viteBuild.default || viteBuild);
