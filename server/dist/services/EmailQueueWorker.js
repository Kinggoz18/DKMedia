...
class EmailQueueWorker {
    constructor(logger) {
        ...
        this.DELAY_QUEUE_NAME = 'email_queue_delay_v1';
        this.DELAY_EXCHANGE_NAME = 'email_exchange_delay';
        this.ROUTING_KEY = 'email';
        this.isConsuming = false;
        this.maxRetries = 3;
        this.FROM_EMAIL = process.env.RESEND_FROM_EMAIL || 'info@dkmedia305.com';
        this.RETRY_DELAY_MS = 5 * 60 * 1000; // 5 minutes for transient failures
        this.logger = logger;
    ...
    calculateTimeUntilMidnight() {
        ...
        const nextMidnight = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 1, 0, 0, 5, 0 // 00:00:05 UTC
        ));
     ...
     */
    async routeToDelayQueue(job, delayMs, reason) {
        if (!this.channel) {
            throw new Error('Channel not available');
        }
        ...
        const published = this.channel.publish(this.DELAY_EXCHANGE_NAME, this.ROUTING_KEY, message, options);
        if (published) {
            this.logger?.info(`Email job ${job.id} routed to delay queue. Reason: ${reason}. Delay: ${delayMs}ms (${Math.round(delayMs / 1000)}s). Expires at: ${new Date(expirationTime).toISOString()}`);
        }
        else {
