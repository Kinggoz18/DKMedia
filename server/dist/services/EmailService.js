...
class EmailService {
     ...
     */
    async scheduleEmail(mailOptions, emailType, scheduledTime) {
        try {
     ...
     */
    async sendBulkEmail(recipients, subject, html, text, emailType = 'newsletter') {
        const errors = [];
        let sent = 0;
        let scheduled = 0;
        ...
        try {
            ...
            const sendPromises = recipientsToSend.map(async (recipient) => {
                try {
                    ...
                    const result = await this.sendEmail(mailOptions, emailType);
                    if (result.success) {
                        sent++;
                    }
     ...
     */
    async scheduleBulkEmail(recipients, subject, html, scheduledTime, text, emailType = 'newsletter') {
        const errors = [];
        let scheduled = 0;
        const total = recipients.length;
        try {
            ...
            for (const recipient of recipients) {
                try {
                    ...
                    await this.scheduleEmail(mailOptions, emailType, scheduledTime);
                    scheduled++;
                }
                catch (error) {
                    errors.push(`Failed to schedule email to ${recipient}: ${error.message}`);
