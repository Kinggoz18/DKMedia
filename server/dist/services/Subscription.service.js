...
export class SubscriptionService {
    constructor(dbCollection, logger) {
         ...
         * @returns
         */
        this.deleteSubscription = async (request, reply) => {
            try {
                const { id } = request.params;
                const subscriptionToDelete = await this.dbCollection.deleteOne({ _id: new ObjectId(id) });
                if (subscriptionToDelete.deletedCount != 1)
                    throw new ReplyError("Subscription not deleted", 404);
         ...
         * @returns
         */
        this.getSubscription = async (request, reply) => {
            try {
                const { id } = request.params;
                const getSubscription = await this.dbCollection.findOne({ _id: new ObjectId(id) });
                if (!getSubscription)
                    throw new ReplyError("Subscription not found", 404);
         ...
         * @returns
         */
        this.getAllSubscription = async (request, reply) => {
            try {
         ...
         * @returns
         */
        this.unsubscribeByEmail = async (request, reply) => {
            try {
