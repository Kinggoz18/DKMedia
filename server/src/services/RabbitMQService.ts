...
export interface EmailJob {
  ...
  from?: string;
  emailType: string;
  expiresAt?: string | Date; // ISO string when serialized, Date when parsed
  scheduledTime?: string | Date; // ISO string when serialized, Date when parsed
  attempts?: number;
}
...
class RabbitMQService {
  ...
  private readonly DLX_NAME = 'email_dlx'; // Dead Letter Exchange
  private readonly DLQ_NAME = 'email_dlq'; // Dead Letter Queue
   ...
   */
  async publishEmailJob(job: EmailJob, delay?: number): Promise<boolean> {
    try {
      await this.connect();
   ...
   */
  async publishBulkEmailJobs(jobs: EmailJob[]): Promise<{ published: number; failed: number }> {
    let published = 0;
    let failed = 0;
   ...
   */
  async getQueueStats(): Promise<{ queue: number; dlq: number }> {
    try {
      await this.connect();
...
export default (logger?: FastifyBaseLogger) => new RabbitMQService(logger);
